...
def concept_from_alias(alias: dict[str, Any]) -> dict[str, Any]:
    name = str(alias.get("name") or "")
    normalized = normalize_title(name)
    ctype = str(alias.get("alias_type") or concept_type(name))
    return {
        "concept_id": alias.get("target_concept_id") or stable_hash(f"concept:{ctype}", normalized),
        "name": name,
        "normalized_name": normalized,
        "concept_type": ctype,
        "confidence": float(alias.get("confidence") or 0.72),
    }


def paper_concept_from_alias(paper_id: str, paper: dict[str, Any], alias: dict[str, Any]) -> dict[str, Any]:
    evidence = f"{paper.get('title') or ''}. {str(paper.get('abstract') or '')[:220]}"
    return {
        "paper_id": paper_id,
        "concept_id": alias.get("target_concept_id"),
        "evidence_field": f"alias_{alias.get('source') or 'phrase'}",
        "evidence_text": evidence[:320],
        "confidence": min(0.95, float(alias.get("confidence") or 0.72)),
        "extractor": "alias_graph_quality",
    }
    ...
    try:
        for paper in source_iter:
            ...
            for alias in aliases:
                ...
                alias_concept = concept_from_alias(alias)
                concept_batch_by_id[alias_concept["concept_id"]] = alias_concept
                paper_concepts.append(paper_concept_from_alias(paper_id, paper_row, alias))
...
def build_aliases(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for paper in source_iter:
        ...
        for alias in aliases:
            ...
            alias_concept = concept_from_alias(alias)
            concept_batch_by_id[alias_concept["concept_id"]] = alias_concept
            paper_concept_batch.append(paper_concept_from_alias(paper_id, paper_row, alias))
        stats["paper_rows"] += 1
        stats["concept_rows"] += len(concepts)
        stats["alias_rows"] += len(aliases)
