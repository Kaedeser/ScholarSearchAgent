...
from scholar_common.io import ensure_paths_exist, load_chunks_by_paper, load_papers
from scholar_common.models import Candidate, Paper, SearchAction
from scholar_common.text import best_snippet, cosine_sparse, token_counter, tokenize


INGESTION_MODULE_ROOT = Path(__file__).resolve().parents[1] / "data_ingestion_indexing"
if INGESTION_MODULE_ROOT.exists() and str(INGESTION_MODULE_ROOT) not in sys.path:
    sys.path.insert(0, str(INGESTION_MODULE_ROOT))
...
class LocalCorpus:
    ...
    def _candidate_from_hit(self, hit: RetrievalHit, action: SearchAction) -> Candidate:
        ...
        candidate = Candidate(
            ...
            metadata={"source_query": action.query, "paper_source": paper.source},
        )
        return candidate
...
class DatabaseCorpus:
    ...
    def __init__(self) -> None:
        from scholar_ingest.config import Settings
        from scholar_ingest.es import ElasticsearchClient
        from scholar_ingest.mysql import MySQLClient
        from scholar_ingest.qdrant import QdrantClient
    ...
    def _candidate_from_es_chunk(self, hit: dict, action: SearchAction) -> Candidate:
        ...
        return Candidate(
            ...
            metadata={"source_query": action.query, "chunk_id": hit.get("chunk_id"), "index": hit.get("_index")},
        )
    ...
    def _candidate_from_qdrant(self, hit: dict, action: SearchAction) -> Candidate:
        ...
        return Candidate(
            ...
            snippets=[],
            metadata={"source_query": action.query, "chunk_id": payload.get("chunk_id"), "point_id": hit.get("id")},
        )
